...
int board_init(void)
{
...
#ifdef CONFIG_FEC_MXC
	//Snehix board does not have FEC
	//setup_fec();
#endif
	...
	setup_epdc();
       ...
       setup_spi();
#endif
	printf("%s Snehix Build 1st February,2022\n", __func__);
...
int power_init_board(void)
{
	...
	/* change sw3 mode to avoid DDR power off */
	printf("PMIC: PFUZE3000 setting the bits SWBST1MODE and SWBST1STBYMODE to 0b10\n");
	swbstctl = pmic_reg_read(dev, PFUZE3000_SWBSTCTL);
	ret = pmic_reg_write(dev, PFUZE3000_SWBSTCTL, swbstctl | 0x48);
	if (ret < 0)
		printf("PMIC: PFUZE3000 change swbstctl failed\n");
